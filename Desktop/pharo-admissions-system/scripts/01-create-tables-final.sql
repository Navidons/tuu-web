...
-- Create the users table for role management (CRITICAL - CREATE FIRST)
...
CREATE POLICY "Admins can insert applications" ON applications
    FOR INSERT WITH CHECK (
        public.get_user_role(auth.uid()) IN ('admin', 'clerk')
    );

CREATE POLICY "Admins can update applications" ON applications
    FOR UPDATE USING (
        public.get_user_role(auth.uid()) IN ('admin', 'principal')
    );
