-- ENABLE ALL FUNCTIONALITIES - Complete System Activation (FIXED)
...
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO students (
            ...
            student_data->>'guardian_occupation',
            student_data->>'guardian_address',
            student_data->>'emergency_contact_name',
            student_data->>'emergency_contact_phone',
            student_data->>'emergency_contact_relationship',
            student_data->>'medical_conditions',
            student_data->>'allergies',
            student_data->>'medications',
            student_data->>'blood_type',
            student_data->>'doctor_name',
            student_data->>'doctor_phone',
            student_data->>'previous_school',
            student_data->>'previous_school_address',
            student_data->>'last_grade_completed',
            student_data->>'academic_achievements',
            student_data->>'special_needs',
            student_data->>'learning_disabilities',
            student_data->>'behavioral_notes',
            COALESCE(student_data->>'transportation_method', 'Parent Drop-off'),
            COALESCE((student_data->>'lunch_program')::BOOLEAN, FALSE),
            student_data->>'extracurricular_interests',
            COALESCE(student_data->>'fee_plan', 'Full Payment'),
            COALESCE((student_data->>'scholarship_applied')::BOOLEAN, FALSE),
            COALESCE((student_data->>'financial_aid_needed')::BOOLEAN, FALSE)
...
-- Step 3: Add sample approved applications (only if they don't exist)
DO $$
DECLARE
    existing_count INTEGER;
BEGIN
    RAISE NOTICE 'Step 3: Adding sample data...';
    
    -- Check if we already have approved applications
    SELECT COUNT(*) INTO existing_count FROM applications WHERE status = 'approved';
    
    -- Only add sample data if we have fewer than 3 approved applications
    IF existing_count < 3 THEN
        -- Add sample approved applications for testing enrollment
        ...
        ) AS new_apps(full_name, email, phone, gender, nationality, status, application_source, priority_score, documents_verified, created_at)
        WHERE NOT EXISTS (
            SELECT 1 FROM applications WHERE email = new_apps.email
        );
        
        RAISE NOTICE '✓ Sample data added';
    ELSE
        RAISE NOTICE '✓ Sample data already exists (% approved applications)', existing_count;
    END IF;
END $$;

-- Step 4: Create sample workflow data
DO $$
BEGIN
    RAISE NOTICE 'Step 4: Creating workflow data...';
...
-- Step 5: Enable all table permissions
DO $$
BEGIN
    RAISE NOTICE 'Step 5: Setting up permissions...';
...
DO $$
BEGIN
    RAISE NOTICE 'Step 6: Creating automation triggers...';
